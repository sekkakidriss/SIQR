function [S,I,Q,R]=ModelMtl(A,beta,mu,alpha1,alpha2,lambda,rho,epsi,p,n,tf,sig11,sig12,sig21,sig22,sig31,sig32,sig41,sig42)
%%%%%%%%%%%%%%%%%%%%%% Initial values %%%%%%%%%%%%%%%%%%%%%%%%%%%%
s0=35435449;
i0=49565;
q0=0;
...
%%%%%%%%%%%%%%%%%%%%%%%%%% Systeme dynamique %%%%%%%%%%%%%%%%%%%%%%%%%
for i=1 : n
  S(i+1) = S(i)+(A-(mu+p*(1-exp(-i)))*S(i)-beta*(a+(1-a)*exp(-0.05*i))*S(i)*I(i)/(1+r1*I(i)+r2*I(i)^2))*h+S(i)*(sig11+sig12*S(i))*sqrt(h)*randn;
  ...
  I(i+1) = I(i) +(beta*(a+(1-a)*exp(-0.05*i))*S(i)*I(i)/(1+r1*I(i)+r2*I(i)^2)-(mu+alpha1+lambda+rho)*I(i))*h+I(i)*(sig21+sig22*I(i))*sqrt(h)*randn;
